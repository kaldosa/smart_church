...
<ul>
<li class="title"><c:out value="${news.subject}" escapeXml="true"/></li>
<li class="date"><s:eval expression="news.createdDate" /></li>
</ul>
</div>
<div class="body_wrap">
<div class="text_contents_wrap">
<div class="text_contents"><s:eval expression="news.contents.replaceAll('\r\n', '<br/>')"/></div>
</div>
</div>
<c:if test="${not empty news.attach}">
<div class="attach_list_box">
<span class="attach_file"><a href="<c:url value="${news.attach.path}"/>">${news.attach.fileName}</a></span>
</div>
</c:if>
</div><!--// 내용보기 -->
...
<a href="<c:url value="/church/${ourChurch.id}/news?criteria=${queryParam.criteria}&keyword=${queryParam.keyword}&page=${queryParam.paginate.page}" />" class="list_btn"><img src="<c:url value="/resources/images/church/common/btn_list3.gif" />" alt="목록" width="72" height="24"/></a>
<sec:authorize ifAnyGranted="ROLE_ADMIN, ROLE_${ourChurch.path}">
<c:url var="deleteUrl" value="/church/${id}/news/${news.id}" />
...
<a href="<c:url value="/church/${ourChurch.id}/news/${news.id}/edit" />" class="edit_btn"><img src="<c:url value="/resources/images/church/common/btn_edit.png" />" alt="목록" width="73" height="22"/></a>
</sec:authorize>
</div>
