...
<ul>
<li class="title title-width-author"><c:out value="${entity.subject}" escapeXml="true"/></li>
<li class="date"><s:eval expression="entity.createdDate" /></li>
<li class="author">${entity.user.name}</li>
</ul>
</div>
<div class="body_wrap">
<div class="text_contents_wrap">
<div class="text_contents"><s:eval expression="entity.contents.replaceAll('\r\n', '<br/>')"/></div>
</div>
</div>

</div><!--// 내용보기 -->

<sec:authorize access="isAuthenticated() and hasPermission(${ourChurch.id}, 'isOurChurchAdmin') or (hasPermission(${ourChurch.id}, 'isChurchMember') and hasPermission(${entity.id}, 'isPrayWriter'))">
<div class="action_btn_area">
<a href="<c:url value="/church/${ourChurch.id}/pray?criteria=${queryParam.criteria}&keyword=${queryParam.keyword}&page=${queryParam.paginate.page}" />" class="list_btn"><img src="<c:url value="/resources/images/church/common/btn_list3.gif" />" alt="목록" width="72" height="24"/></a>

<c:url var="deleteUrl" value="/church/${ourChurch.id}/pray/${entity.id}" />
<form:form action="${deleteUrl}" method="delete" cssClass="delete-pray-form">
...
<a href="<c:url value="/church/${ourChurch.id}/pray/${entity.id}/edit" />" class="edit_btn"><img src="<c:url value="/resources/images/church/common/btn_edit.png" />" alt="목록" width="73" height="22"/></a>
</div>
</sec:authorize>
...
<c:url var="addCommentUrl" value="/church/${ourChurch.id}/pray/${entity.id}"/>
...
<div class="comm_wrap"> 
<h5>한줄댓글</h5>
<form:textarea path="comments" cols="77" rows="4" cssClass="input_textarea" id="comment-textarea"/>
...
<c:forEach var="comment" items="${entity.comments}" varStatus="status">
    <div class="item">
        ...
        <dl class="comment">
            ...
            <span><a href="<c:url value="/church/${ourChurch.id}/pray/${entity.id}/${comment.id}"/>" class="comment-del">삭제</a></span>
            </sec:authorize>
            </dd>
        </dl>
    </div>
...
<input type="hidden" name="spring-security-redirect" value="/church/${ourChurch.id}/pray/${entity.id}?criteria=${queryParam.criteria}&keyword=${queryParam.keyword}&page=${queryParam.paginate.page}" />
</fieldset>
</form>
...
$('.delete-pray-form').submit(function() {
    if(!confirm("삭제하시겠습니까?")) {
        return false;
    }
});
