...
(function ($) {
    ...
    function Timepicker() {
        ...
        this._defaults = { // Global defaults for all the time picker instances
            ...
            showPeriodPosition: false		// AM/PM을 시간 앞 혹은 뒤에 나타낼지를 위한 설정 (기본 false : 00:00 AM)
        ...
        */
        parseTime: function (inst, timeVal) {
            ...
            var timeSeparator = this._get(inst, 'timeSeparator'),
                ...
                showPeriod = (this._get(inst, 'showPeriod') == true),
                showPeriodPosition = (this._get(inst, 'showPeriodPosition') == true),
                periodSeparator = this._get(inst, 'periodSeparator'),
                p = timeVal.indexOf(timeSeparator);

            // check if time separator found
            if (p != -1) {
            	if(showPeriodPosition) {
            		var time = timeVal.split(periodSeparator);
            		tp = time[1].indexOf(timeSeparator);
            		retVal.hours = parseInt(time[1].substr(0, tp), 10);
            		retVal.minutes = parseInt(time[1].substr(tp + 1), 10);
            	} else {
        ...
        _getParsedTime: function(inst) {
            ...
            if (showHours) {
                ...
                if (period.length > 0) {
                	if(showPeriodPosition) {
                		displayTime = period + this._get(inst, 'periodSeparator') + parsedTime; 
                	} else {
                		displayTime = parsedTime + this._get(inst, 'periodSeparator') + period;
                	}
                }
            }
